...
% Fonction utilisée pour calculer la consommation du noeud dans
% l'algorithme d'optimisation pour la configuration électronique
% Cette fonction consiste à encadrer la consommation entre deux valeurs min
% et max pour vérifier si elle appartient au domaine de consommation défini
% Si c'est le cas, le calcul détaillé est fait 
% Cette approche réduit le temps de simulation 
...
% Consommation désirée / Autonomie désirée
conso_desiree = e3;

% Marge de consommation acceptable
marge = e4; 

% taille de données maximale à envoyer 
load ('initialisation','data_max')

% Variables à optimiser
connexion = e5 ; % Période de transmission 
var1 = e6 ; % Période de mesure 
var2 = e7 ; % fréquence CPU
var3 = e8; % Puissance de transmission RF 
var4 = e9; % Fréquence d'échantillonnage 
 
%-----------------------------------------------------%
% Caractéristiques de la mémoire 
% But : déterminer la stratégie adoptée pour la mémorisation des données
...
n_cycles_coupure = sum (CR(:)==-1); % Nombres de cycles avec connexion établie 
n_cycles_mesure = sum (CR(:)==-2); % Nombres de cycles sans connexion établie
...
%------------------- Calcul du maximum --------------------%
% Consommation pour un cycle avec une connection établie
...
% Consommation un cycle de coupure réseau
...
%------------------- Calcul du minimum --------------------%
% Consommation pour le premier cycle de coupure réseau 
